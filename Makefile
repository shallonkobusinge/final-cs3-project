...
STUDENT_LIBS = vector list polygon body scene color forces collision emscripten sdl_wrapper landing_page end_page maze 
...
DEMO_REF = color emscripten list polygon sdl_wrapper vector body scene forces collision asset_cache asset landing_page  cell stack maze seeker sound_effect end_page
...
GAME_STUDENT = game sdl_wrapper asset asset_cache landing_page  cell stack maze seeker sound_effect end_page
GAME_STUDENT_OBJS = $(addprefix out/,$(GAME_STUDENT:=.wasm.o))

TEST_REF = asset_cache asset landing_page cell stack maze seeker sound_effect end_page
