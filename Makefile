...
STUDENT_LIBS = vector list polygon body scene color forces collision emscripten sdl_wrapper landing_page maze 
...
DEMO_REF = color emscripten list polygon sdl_wrapper vector body scene forces collision asset_cache asset landing_page cell stack maze seeker sound_effect
DEMO_REF_OBJS = $(addprefix $(REF_FOLDER)/,$(DEMO_REF:=.wasm.ref.o))

GAME_REF = color emscripten list polygon sdl_wrapper vector body scene forces collision asset_cache asset landing_page cell stack maze seeker sound_effect
GAME_REF_OBJS = $(addprefix $(REF_FOLDER)/,$(GAME_REF:=.wasm.ref.o))

GAME_STUDENT = color emscripten list polygon sdl_wrapper vector body scene forces collision asset_cache asset landing_page cell stack maze seeker sound_effect
GAME_STUDENT_OBJS = $(addprefix out/,$(GAME_STUDENT:=.wasm.o))

TEST_REF = color emscripten list polygon sdl_wrapper vector body scene forces collision asset_cache asset landing_page cell stack maze seeker sound_effect
