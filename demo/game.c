...
state_t *emscripten_init()
{
    ...
    asset_t *asset_bg_img =
        asset_make_image(BACKGROUND_PATH, (SDL_Rect){0, 0, MAX.x, MAX.y});
    list_add(state->body_assets, asset_bg_img);

    asset_t *asset_frog = asset_make_image_with_body(FROGGER_PATH, froggy);
    list_add(state->body_assets, asset_frog);
    ...
    for (size_t r = 3; r < ROWS + 3; r++)
    {
        ...
        while (cx < MAX.x)
        {
            ...
            asset_t *asset_log = asset_make_image_with_body(LOG_PATH, obstacle);
            list_add(state->body_assets, asset_log);
        }
    }
